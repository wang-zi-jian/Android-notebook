...
public class MainActivity extends AppCompatActivity{
    ...
    private void InitNote() {       //进行数据填装
        ...
        Cursor cursor  = db.rawQuery("select * from Note",null);    //游标类查询指定内容】【】

        IDList.clear();
        TADList.clear();        //清空两个list
        while(cursor.moveToNext()){   //进行下一行
    ...
    public void RefreshTADList(){       //返回该界面时刷新的方法
        ...
        while(cursor.moveToNext()){         //对两个list重新赋予值
            int id = cursor.getInt(cursor.getColumnIndex("id"));      //获取数据库内容值，列值的id
            String title = cursor.getString(cursor.getColumnIndex("title"));
            String date = cursor.getString(cursor.getColumnIndex("date"));                //获取墅区库所有数据
            IDList.add(id);
            TADList.add(title+"\n"+ date);      //将title和时间分开显示
        }
    }
       ...
       // InitNote();
        ...
        ButtonAdd.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View v){
                Intent intent = new Intent(MainActivity.this, ch.neet.wzj.notebook.Add.class);  //启动add
...
//                    Intent intent = new Intent(MainActivity.this, ch.neet.wzj.notebook.Research.class);
...
//
        ...
        ListView.setOnItemClickListener(new AdapterView.OnItemClickListener(){      //配置ArrayList点击按钮
            @Override
            public void  onItemClick(AdapterView<?> parent, View view , int position , long id){  //parent配置器，id是被点击第几行的位置
                int tran = IDList.get(position);        //点击不同的行，返回不同的id
                Intent intent = new Intent(MainActivity.this, ch.neet.wzj.notebook.Edit.class);
                intent.putExtra("tran",tran);
                startActivity(intent);      //通过intent传输，提交activity值
            }
        });
